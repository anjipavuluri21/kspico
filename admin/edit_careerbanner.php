...
{			
		...
		{			
			...
			$increment2 = '';
				...
				//$name2 = pathinfo($_FILES['career_banner']['name'], PATHINFO_FILENAME);
				$name2 = "career";
				$extension2 = pathinfo($_FILES['career_banner']['name'], PATHINFO_EXTENSION);
				...
				if (move_uploaded_file($_FILES['career_banner']['tmp_name'], $target_file2)) {					
					$career_banner = $blocation2;										
				}
		}
		...
		{			
			...
			if($id != 0) {
				if($career_banner != "") {
					$hea_banner = "career_banner= '$career_banner'";
				}
				else {
					$hea_banner = "";
				}
				if($side_banner != "") {
					$aw_banner = "careerside_banner= '$side_banner'";
				}
				else {
					$aw_banner = "";
				}
				...
				$updatesql = mysqli_query($connection,"insert into sumc_banners (career_banner,careerside_banner) values ('$career_banner','$side_banner')");
			}
			if($updatesql) {
				header("location:edit_careerbanner.php?msg=success");
				exit;
			}
}
$career_banner="";
...
$careerside_banner = $booth_features_result['careerside_banner'];
$career_banner = $booth_features_result['career_banner'];
	...
	?>
        ...
        <div class="container-fluid">
          ...
          <div class="d-sm-flex align-items-center justify-content-between mb-4">
            <h1 class="h3 mb-0 text-gray-800">Career Banners</h1>
          </div>
          <div class="row">
            <div class="col-lg-12 mb-12">
              <div class="card shadow mb-8">
                <div class="card-header py-3">
                  <h6 class="m-0 font-weight-bold text-primary">View Career Banners</h6>
                </div>
                <div class="card-body">
				<form action="" method="post" name="adv_form" id="adv_form" enctype="multipart/form-data" data-parsley-validate class="form-horizontal form-label-left">
                    <div class="form-group">
							...
							<?php
							if(isset($careerside_banner) && $careerside_banner != "") {
								?>
								<img src='../<?php echo $careerside_banner; ?>' height='150px' width='150px;' />
								<?php
							}
							?>
							</div>
				  ...
				  <div class="form-group">
							<label class="control-label col-md-3 col-sm-3 col-xs-12" for="last-name">Career Image</label>
							<div class="col-md-6 col-sm-6 col-xs-12">
							<input type="file" name="career_banner" id="career_banner" class="form-control" />
							<p id="note">Please upload image with size 2000X500</p>
							<?php
							if(isset($career_banner) && $career_banner != "") {
								?>
								<img src='../<?php echo $career_banner; ?>' height='150px' width='450px;' />
								<?php
							}
							?>
							</div>
