	...
	?>
        ...
        <div class="container-fluid">
          ...
          <div class="d-sm-flex align-items-center justify-content-between mb-4">
			...
			<a href="edit_homeslider.php" class="d-none d-sm-inline-block btn btn-sm btn-primary shadow-sm"><i class="fas fa-check fa-sm text-white-50"></i> Add Home Sliders</a>
          </div>
                ...
                <div class="card-body">
				  ...
				  <form name="submenuform" method="post" action="banner_ordering.php">
                    <table class="table table-bordered" id="example" width="100%" cellspacing="0">
                      ...
                      <tbody>
					  <?php
					  $project_features_sql = mysqli_query($connection,"select * from sumc_homeslides order by id desc ");
					  if(mysqli_num_rows($project_features_sql) > 0) {
						  ...
						  while($project_features_result = mysqli_fetch_array($project_features_sql)) {
							  ...
							  $title = $project_features_result['title'];
                                                          $text_title = $project_features_result['text_title'];
							  ...
							  <td><?php echo $i; ?></td>
							  <!--<td>
							  <input type="text" class="form-control" name="order[]" id="order[]" value="<?php echo $project_features_result['ordering']; ?>" oninput="this.value=this.value.replace(/[^0-9]/g,'');" style='width:50px;' />
									<input type="hidden" name="menuid[]" id="menuid" value="<?php //echo $banner_result[id] ?>">
							  
							  </td>-->
                                                          <td><?php echo $text_title; ?></td>
							  <td><?php echo $title; ?></td>
							  
							  <td><img src='../<?php echo $project_features_result['slide_image']; ?>' height='100px' width='150px;' /></td>
							  ...
							  <td><a href="edit_homeslider.php?id=<?php echo $id; ?>" class="btn-sm btn-primary">Edit</a>&nbsp;
...
$("[id^='delete_']").click(function(){
     	var i=$(this).attr('id');		
   	 	var arr=i.split("_");
   	 	var i=arr[1];
   	 	var r=confirm("Are you sure you want to delete the Banner?");
   	 	if(r==true)
   	 	{	
   	 	 $.ajax({
			      	type:"POST",
			      	data:"id="+i,
			      	url:"deletecollection.php?type=sumc_homeslides",
...
$("[id^='status_']").click(function(){
		...
		var j=arr2[1];
   	 	...
   	 	{   	 		
   	 	 $.ajax({
			      	type:"POST",
			      	data:"id="+i+"&"+j+"&tab=sumc_homeslides",
