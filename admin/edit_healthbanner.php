...
{			
		...
		{			
			...
			$increment2 = '';
				...
				$name2 = "health";
				$extension2 = pathinfo($_FILES['health_banner']['name'], PATHINFO_EXTENSION);
				...
				if (move_uploaded_file($_FILES['health_banner']['tmp_name'], $target_file2)) {					
					$health_banner = $blocation2;										
				}
		}
		...
		{			
			...
			if($id != 0) {
				if($health_banner != "") {
					$hea_banner = "health_banner= '$health_banner'";
				}
				else {
					$hea_banner = "";
				}
				if($side_banner != "") {
					$aw_banner = "healthside_banner= '$side_banner'";
				}
				else {
					$aw_banner = "";
				}
				...
				$updatesql = mysqli_query($connection,"insert into sumc_banners (health_banner,healthside_banner) values ('$health_banner','$side_banner')");
			}
			if($updatesql) {
				header("location:edit_healthbanner.php?msg=success");
				exit;
			}
}
$health_banner="";
...
$healthside_banner = $booth_features_result['healthside_banner'];
$health_banner = $booth_features_result['health_banner'];
	...
	?>
        ...
        <div class="container-fluid">
          ...
          <div class="d-sm-flex align-items-center justify-content-between mb-4">
            <h1 class="h3 mb-0 text-gray-800">Health Banners</h1>
          </div>
          <div class="row">
            <div class="col-lg-12 mb-12">
              <div class="card shadow mb-8">
                <div class="card-header py-3">
                  <h6 class="m-0 font-weight-bold text-primary">View Health Banners</h6>
                </div>
                <div class="card-body">
				<form action="" method="post" name="adv_form" id="adv_form" enctype="multipart/form-data" data-parsley-validate class="form-horizontal form-label-left">
                    <div class="form-group">
							...
							<?php
							if(isset($healthside_banner) && $healthside_banner != "") {
								?>
								<img src='../<?php echo $healthside_banner; ?>' height='150px' width='450px;' />
								<?php
							}
							?>
							</div>
				  ...
				  <div class="form-group">
							<label class="control-label col-md-3 col-sm-3 col-xs-12" for="last-name">Health Image</label>
							<div class="col-md-6 col-sm-6 col-xs-12">
							<input type="file" name="health_banner" id="health_banner" class="form-control" />
							<p id="note">Please upload image with size 2000X500</p>
							<?php
							if(isset($health_banner) && $health_banner != "") {
								?>
								<img src='../<?php echo $health_banner; ?>' height='150px' width='450px;' />
								<?php
							}
							?>
							</div>
