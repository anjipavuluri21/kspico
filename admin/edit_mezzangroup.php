...
{		
	...
	if($id == 0) {
		$login_query = mysqli_query($connection,"insert into sumc_mezzan_group (content,content2,highlights,image,status,created_date) values ('$content','$content2','$highlights','$image',1,NOW())");
	}
	else {	
		if($image != "") {
		...
		$login_query = mysqli_query($connection,"UPDATE sumc_mezzan_group SET  content='$conent',content2='$conent2',highlights='$highlights' $corse_img  WHERE id ='$id'");		
	}
	if($login_query == 1){
		header("location:edit_mezzangroup.php?msg=success");		
	} else {		
		header("location:edit_mezzangroup.php?msg=fail");		
	}	
}
...
$features_sql = mysqli_query($connection,"select * from sumc_mezzan_group");
