...
{			
			...
			$increment_icon = '';
				...
				//start with no suffix
				while (file_exists("../uploads/companygm/" . $name_icon . $increment_icon . '.' . $extension_icon)) {
					$increment_icon++;
				}
				
				$basename_icon = $name_icon . $increment_icon . '.' . $extension_icon;

				$blocation_icon = "uploads/companygm/" . $basename_icon;

				$target_file_icon = "../uploads/companygm/" . $basename_icon;
			...
			if($id != 0) {
				...
				$updatesql = mysqli_query($connection,"update sumc_gm_message set description='$description' $corse_img $side_img where id=".$id);
				
			}
			else {
				
				$updatesql = mysqli_query($connection,"insert into sumc_gm_message (description,status,created_date) values ('$description',1,NOW())");
				
				
			}
			if($updatesql) {
				header("location:edit_gmmessage.php?msg=success");
				exit;
			}
}
...
if($id != 0) {
$features_sql = mysqli_query($connection,"select * from sumc_gm_message where id=".$id);
	...
	?>
        ...
        <div class="container-fluid">
          ...
          <div class="d-sm-flex align-items-center justify-content-between mb-4">
            <h1 class="h3 mb-0 text-gray-800">MESSAGE FROM G.M.</h1>
          </div>
          <div class="row">
            <div class="col-lg-12 mb-12">
              <div class="card shadow mb-8">
                <div class="card-header py-3">
                  <h6 class="m-0 font-weight-bold text-primary">View MESSAGE FROM G.M.</h6>
                </div>
                <div class="card-body">
