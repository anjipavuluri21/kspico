...
{		
	...
	if($_FILES['image']['name'] != "")
			...
			$increment_icon = '';
				...
				while (file_exists("../uploads/diverse/" . $name_icon . $increment_icon . '.' . $extension_icon)) {
					$increment_icon++;
				}
				
				$basename_icon = $name_icon . $increment_icon . '.' . $extension_icon;

				$blocation_icon = "uploads/diverse/" . $basename_icon;

				$target_file_icon = "../uploads/diverse/" . $basename_icon;
	...
	if($id == 0) {
		$login_query = mysqli_query($connection,"insert into sumc_diverse (title,image,content,status,created_date) values ('$title','$image','$content',1,NOW())");
	}
	else {	
		if($image != "") {
		...
		$login_query = mysqli_query($connection,"UPDATE sumc_diverse SET  title='$title',content='$content' $corse_img  WHERE id ='$id'");		
	}
	if($login_query == 1){
		header("location:diverselist.php?msg=success");		
	} else {		
		header("location:diverselist.php?msg=fail");		
	}	
}
...
if($id != 0) {
$features_sql = mysqli_query($connection,"select * from sumc_diverse where id=".$id);
	...
	?>
        ...
        <div class="container-fluid">
          ...
          <div class="d-sm-flex align-items-center justify-content-between mb-4">
               <h1 class="h3 mb-0 text-gray-800">Diverse PHARMACEUTICAL COMPANY </h1>
              </div>
          <div class="row">
            <div class="col-lg-12 mb-12">
              <div class="card shadow mb-8">
                <div class="card-header py-3">
                    <h6 class="m-0 font-weight-bold text-primary">Edit Diverse</h6>                    
                    </div>
                <div class="card-body">
					...
					<div class="form-group">
						...
						<div class="col-md-9 col-sm-9 col-xs-12">
							<textarea name="content" id="description" required="required" class="form-control"><?php echo $content; ?></textarea>
						</div>
                    </div>                      
                      ...
                      <div class="form-group">
						...
						<a href="diverselist.php" class="btn btn-primary">Cancel</a>
